...
{
    /* Your code goes here! */
    string word[2] = {word1, word2};
    transform(word[0].begin(),word[0].end(),word[0].begin(),::toupper);
    transform(word[1].begin(),word[1].end(),word[1].begin(),::toupper);
    auto it = dict.find(word[0]);
    if (it == dict.end()) {
        return false;
    }
    vector<string> output = it->second;
    it = dict.find(word[1]);
    if(it == dict.end()) {
        return false;
    } 
    if (it ->second.size() != output.size()) {
        return false;
    }
     for(unsigned i = 0; i < output.size(); i++) {
        if(output[i] != it ->second[i]) {
            return false;
        }
    }
    return true;
