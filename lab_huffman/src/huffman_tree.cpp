                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(singleQueue.empty() && mergeQueue.empty()) {
        ...
        TreeNode* output = mergeQueue.front();
        mergeQueue.pop();
        return output;
    } else if(mergeQueue.empty()) {
        TreeNode* output = singleQueue.front();
        singleQueue.pop();
        return output;
    } else {
        TreeNode* first = singleQueue.front();
        TreeNode* second = mergeQueue.front();
        if(first->freq.getFrequency() < second->freq.getFrequency()) {
            singleQueue.pop();
            return first;
        } else if(first->freq.getFrequency() == second->freq.getFrequency()) {
            singleQueue.pop();
            return first;
        } else {
            mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for(unsigned i = 0; i < frequencies.size(); i++) {
        singleQueue.push(new TreeNode(Frequency(frequencies[i])));
    }
    while(singleQueue.size() + mergeQueue.size() > 1) {
        ...
        TreeNode* input = new TreeNode(Frequency(l->freq.getFrequency() + r->freq.getFrequency()));
        input ->left  = l;
        input ->right = r;
        mergeQueue.push(input);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit() == true) {
        TreeNode* output = new TreeNode(Frequency(bfile.getNextByte(), 0));
        return output;
    } else {
        TreeNode* output = new TreeNode(Frequency(0));
        output ->left = readTree(bfile);
        output -> right = readTree(bfile);
        return output;
    }
    return NULL;
}
