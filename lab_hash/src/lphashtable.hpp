...
int LPHashTable<K, V>::findIndex(const K& key) const
{
     ...
     */
    unsigned origin = hashes::hash(key,size);
    unsigned idx = origin;
     if (table[idx] != NULL) {
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     * @hint Use findPrime()!
     */
    unsigned size1 = findPrime(size * 2);
    pair<K, V> **temp = new pair<K, V> *[size1];
    delete[] should_probe;
    should_probe = new bool[size1];
    ...
    for (unsigned slot = 0; slot < size; slot++) {
        if (table[slot] != NULL) {
            unsigned idx = hashes::hash(table[slot]->first, size1);
            while (temp[idx] != NULL) {
                idx = (idx + 1) % size1;
            }
            temp[idx] =
                new pair<K, V>(table[slot]->first, table[slot]->second);
            should_probe[idx] = true;
        }
    }
